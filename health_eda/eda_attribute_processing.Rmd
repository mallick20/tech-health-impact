...
final_data <- read_csv("../health_data/data_2002-2007.csv")
```
...
### People Surveyed by Sex
```{r}
num_surveyed_gender <- final_data |>
  mutate(Gender = factor(IRSEX, levels = c(1, 2), labels = c("MALE", "FEMALE"))) |> 
  group_by(YEAR, Gender) |> 
  summarize(num_people=n()) |>
  ungroup() |> 
  pivot_wider(names_from = Gender, values_from = num_people, names_prefix = "Sex_")
```
...
```{r}
psych_summ_ovrl <- nervous_summ_ovrl |> left_join(restless_summ_ovrl) |> left_join(hopeless_summ_ovrl)
...
```{r}
psych_summ_age_ovrl <- nervous_summ_age_ovrl |> left_join(hopeless_summ_age_ovrl, by=c("YEAR", "CATAG7")) |> left_join(restless_summ_age_ovrl, by=c("YEAR", "CATAG7"))
...
```{r}
psych_summ_income_ovrl <- nervous_summ_income_ovrl |> left_join(hopeless_summ_income_ovrl, by=c("YEAR", "INCOME")) |> left_join(restless_summ_income_ovrl, by=c("YEAR", "INCOME"))
...
```{r}
psych_summ_edu_ovrl <- nervous_summ_edu_ovrl |> left_join(hopeless_summ_edu_ovrl, by=c("YEAR", "EDUCCAT2")) |> left_join(restless_summ_edu_ovrl, by=c("YEAR", "EDUCCAT2"))
